...
import com.littlebirds.petshopapp.adapters.EmployeesAdapter;
...
public class EmployeesActivity extends BaseActivity {
    ...
    private static final String URL_EMPLOYEES = "http://10.0.2.2:8080/user?role=WORKER";

    private RecyclerView recyclerEmployees;
    private EmployeesAdapter adapter;

    private List<ClientDto> employees = new ArrayList<>();
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        adapter = new EmployeesAdapter(filtered);
        recyclerEmployees.setAdapter(adapter);

        inputSearch = findViewById(R.id.inputSearchEmployee);
        spinnerStatus = findViewById(R.id.spinnerStatusEmployee);
        spinnerSort = findViewById(R.id.spinnerSortEmployee);
    ...
    private void fetchEmployees() {
        ...
        JsonObjectRequest request = new JsonObjectRequest(
                ...
                error -> {
                    Toast.makeText(this, "Erro ao buscar funcionários: " + error.toString(), Toast.LENGTH_SHORT).show();
    ...
    private void parseEmployees(JSONArray array) {
        try {
            for (int i = 0; i < array.length(); i++) {

                JSONObject obj = array.getJSONObject(i);

                if (!obj.optString("role", "").equalsIgnoreCase("WORKER")) continue;
    ...
    private void applyFilters() {
        ...
        for (ClientDto e : employees) {
            boolean matchName = e.getFullName().toLowerCase().contains(query);
            boolean matchStatus =
                    status.equals("Todos")
                            || (status.equals("Ativo") && e.isActive())
                            || (status.equals("Inativo") && !e.isActive());
